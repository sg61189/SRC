...
def compute_k_clusters_parallel(embeddings, k_range, method='kmedoids', distance_metric='cosine', n_jobs=-1):
    """
...
def main(n,input_dir, method='kmedoids', distance_metric='cosine'):
    #Step 1: Load the pickles hence tensors
    #print (f"At main....")
    embeddings = load_pickled_objects_from_directory(input_dir) 

    #Step 2: 
    cluster_results = compute_k_clusters_parallel(embeddings,range(1, n+1) ,method=method, distance_metric=distance_metric)
    ...
    if cluster_results:
        ...
        for k, clusters in formatted_results.items():
            ...
            print(f"k={k} : {{{cluster_str}}}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Cluster embeddings from pickled files.')
    parser.add_argument('--n', type=int,required=True ,help='Total number of clusters.')
    parser.add_argument('--input_dir', type=str,required=True ,help='Directory containing pickled embedding files.')
    ...
    main(args.n,args.input_dir, method=args.method, distance_metric=args.distance_metric)
