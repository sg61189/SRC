...
def process_circuits(START_FRAME: int, end_time: float, args: any, FLAGS: str, CURRENT_FRAME: int, modified_time: int, k: int):
    ...
    print(
    f"DEPTH ({k+1}): DeepGate2 execution + BMC engine selection: {round(time.time() - time_stamp, 2)} secs\n"
    )
    ...
    if engine is None:
        ...
        print(f"Time wasted: {time_wasted} sec")
        os._exit(1)
    print(
...
def fixed_time_partition_mode(args: any) -> None:
    ...
    CURRENT_FRAME = 0
    line_number = 1
    mabmc_time_path = args.mabmc_time_path
    #luby_i = 1
    k = 0
    ...
    try:
        while time.time() - start_time <= args.total_time:
            ...
            if (
                ...
                print(mabmc_time_path)
                first_iteration = False
                CURRENT_FRAME = START_FRAME
                if first_similarity_check_done is not True:
                    modified_time = float(linecache.getline(mabmc_time_path, line_number).strip())
                    FLAGS = f"-S {START_FRAME} -T {modified_time} -F 0 -v"
                ...
                if first_similarity_check_done:
                    ...
                    if time_wasted < args.p*modified_time:
                        ...
                        if linecache.getline(mabmc_time_path, line_number).strip() == '':
                            modified_time = int(end_time - time.time())
                            FLAGS = f"-S {START_FRAME} -T {modified_time} -F 0 -v"
                            print(f"\nSince no more entry in mabmc_time.txt, running bmc3g for the remaining time: {modified_time} secs\n")
                            depth_reached = run_engine("bmc3g", args.input_circuit, FLAGS)
                            print(f'\n{bmc_data_last_depth}\n')
                            last_depth_data = extract_data(depth_reached) ###
                            time_wasted = round(modified_time - float(last_depth_data[7]), 2)
...
def main():
    ...
    parser.add_argument(
        "-mabmc_time_path", type=str, help="file containing mabmc_time quantums", required=True
    )
    parser.add_argument(
