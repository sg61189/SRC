...
def process_circuits(START_FRAME: int, end_time: float, args: any, FLAGS: str, CURRENT_FRAME: int, modified_time: int, k: int):
    ...
    if engine is None:
        ...
        print(f'\n{bmc_data_last_depth}\n')
        last_depth_data = extract_data(bmc_data_last_depth) ###
        time_wasted = round(modified_time - float(last_depth_data[7]), 2)
        print(f'\nTime wasted: {time_wasted} sec.')
        os._exit(1)
...
def terminate_process(signum: int, frame: any, modified_time: int) -> None:
    global bmc_data_last_depth
    print("=" * 80 + " TIMEOUT " + "=" * 80)
...
def fixed_time_partition_mode(args: any) -> None:
    ...
    signal.signal(signal.SIGALRM, lambda signum, frame: terminate_process(signum, frame, modified_time))
