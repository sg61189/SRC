...
class bandit:
	...
	def run(self):
		...
		for i in range(4*self.iters):
			...
			print('Next time out', self.timeout[i], 'for chosen action', a, Actions[a],  'ocount', ocount, 'flag', flag, 'once', once, 'ending', ending)
			...
			# else:
			#    ss = (Actions[a], -1, reward, -1, self.states)
...
def main(argv):
	...
	labels = [r'$\epsilon= 0.1, \alpha = {0}$'.format(alpha), 'ucb1']


	fname = (inputfile.split('/')[-1]).split('.')[0]
	print(fname)
	pp = PdfPages("plots/plot_MAB_BMC_fixt_{0}_{1}{2}.pdf".format(fname, DIFF, '_FIX' if DIFF else ''))
	j = 0
	all_rewards = []
	all_selection = []
	all_results = []
